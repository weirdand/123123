...
export default function DisplayPanel({ devices, syncEnabled, onSyncToggle, onTaskAssign }: Props) {
  ...
  return (
    <motion.div 
      ...
      className="space-y-4"
    >
      {/* Верхняя панель с настройками (без изменений) */}
        ...
        <div className="glass rounded-xl overflow-hidden">
          ...
          <div className="p-4 flex flex-wrap items-center gap-6">
            ...
            <button 
              ...
              className={`py-2.5 px-4 rounded-lg border flex items-center gap-2 font-mono text-[10px] tracking-widest uppercase transition-all whitespace-nowrap ${
                hidden 
          ...
          <div className="p-4 flex flex-col items-center justify-center gap-3">
            <button 
              ...
              className={`relative w-20 h-10 rounded-full transition-all duration-300 border-2 ${
                syncEnabled ? "border-accent bg-accent/20 neon-glow-blue" : "border-border bg-secondary/50"
              }`}
            >
              <motion.div 
                animate={{ x: syncEnabled ? 28 : 0 }} 
                transition={{ type: "spring", stiffness: 500, damping: 30 }} 
                className={`absolute top-1 left-1.5 w-7 h-7 rounded-full transition-colors duration-300 ${
                  syncEnabled ? "bg-accent" : "bg-muted-foreground"
                }`} 
              />
            </button>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className={`font-mono text-[10px] tracking-widest uppercase font-bold ${
                syncEnabled ? "text-accent" : "text-muted-foreground"
              }`}>
            ...
            {syncEnabled && (
              <motion.span 
                ...
                animate={{ opacity: 1, y: 0 }} 
                className="font-mono text-[9px] text-accent/80 tracking-wider"
              >
                Sync Active: {selectedCount} Devices
              </motion.span>
            )}
          </div>
        </div>
